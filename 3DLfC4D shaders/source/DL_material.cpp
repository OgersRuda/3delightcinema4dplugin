...
class DL_Material : public MaterialData
{
...
public:
	...
	static NodeData* Alloc(void) { return NewObjClear(DL_Material); }

	//Int32 updatecount;
};

Bool DL_Material::Init(GeListNode* node)
{
...
INITRENDERRESULT DL_Material::InitRender(BaseMaterial* mat, const InitRenderStruct& irs)
{
...
void DL_Material::CalcSurface(BaseMaterial* mat, VolumeData* vd)
{
	...
	att_dif = att_spc * color;
	 ...
	 (att_dif * (diff + vd->ambient)) + (att_spc * spec);
